...
data class KotlinIndexPatternOccurrence(private val _file: PsiFile, private val _textRange: TextRange, private val _pattern: IndexPattern) : IndexPatternOccurrence {
    override fun getFile() = _file
    override fun getPattern() = _pattern
    override fun getTextRange() = _textRange
}

class KotlinIndexPatternSearcher : QueryExecutorBase<IndexPatternOccurrence, IndexPatternSearch.SearchParameters>(true) {
            ...
            override fun visitCallExpression(expression: KtCallExpression) {
                if (expression.calleeExpression?.text == "TODO") {
                    ...
                    if (argList?.arguments?.size == 1) {
                        consumer.process(KotlinIndexPatternOccurrence(file, expression.textRange, pattern))
                    }
                }
            }
        })
